...
function getType() {
    //Visible box for DARK GARMENT and Powder
    let darkDiv = document.getElementById("darkDiv");
    let powderDiv = document.getElementById("powderDiv");
    //Inputs
    let type = document.getElementsByName("type");
    ...
    if (sheetSize.value !== '' && darkGarment !== undefined){
        ...
        if(type[0].checked){
            spotColorRate = 0.10;
            darkDiv.style.display = "none";
            powderDiv.style.display = "none";
            numColors.value = Number(1);
            actualColorNumber = numColors.value;
        }else{
            spotColorRate = 0;
            darkDiv.style.display = "block";
            powderDiv.style.display = "block";
        }
        ...
        if (powder.checked || type[0].checked){
        ...
        finalSheetCost = (Number(sheetPrice) + Number(totalAdditionalOptions) + Number(sheetPrice*spotColorRate)).toFixed(2);
        totalPriceColumn.textContent = Number(finalSheetCost * Number(numSheets)).toFixed(2) + " €";
        addOptPrice.textContent = numSheets + " Bögen";
        sheetPriceColumn.textContent = finalSheetCost + " €/Bogen";
